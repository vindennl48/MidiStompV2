...
def serial_connect():
    Debug.Print_nline("Connecting..", 1)
    write(0xff)
    if read() == 0xff:
        Debug.Print_nline("Connected!")
        return True
    raise Exception(f"ERROR>Connection Failed")
    return False

def serial_mode(mode):
    Debug.Print_nline("Setting serial mode..", 1)
    write(mode)
    if read() == 0xff:
        Debug.Print_nline(f"Serial mode set! ({mode})")
        return True
...
def write_block(addr, block):
    ...
    if read() == 0xff:
        return True
    raise Exception(f"ERROR>write_block({addr}) - failed")
